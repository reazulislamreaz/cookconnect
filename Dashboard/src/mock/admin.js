...
export const offerStats = () => {
  ...
  return {
    ...
    expired: JOBS.filter((j) => j.status === "expired" || (j.status === "active" && isExpired(j))).length,
...
/* ------------------------------------------------- registration series */
// Improvement points 5: new candidates per day, new candidates by job position,
// new employers per day and per month, establishments by type.
//
// All four are counted off `registeredAt`, which both fixtures carry. Buckets
// are built from a full date range rather than from the rows, so a day nobody
// signed up on renders as a zero instead of vanishing and making the curve lie.

const dayBefore = (days) => new Date(TODAY.getTime() - days * 86400000);
const isoDay = (d) => d.toISOString().slice(0, 10);

/** `[{ day, count }]` over the last `days` days, oldest first, zeros included. */
const perDay = (rows, days) => {
  const counts = rows.reduce((acc, r) => {
    acc[r.registeredAt] = (acc[r.registeredAt] || 0) + 1;
    return acc;
  }, {});

  return Array.from({ length: days }, (_, i) => {
    const day = isoDay(dayBefore(days - 1 - i));
    return { day, count: counts[day] || 0 };
  });
};

/** `[{ month, count }]` for the twelve months of the fixture's current year. */
const perMonth = (rows) => {
  const year = TODAY.getUTCFullYear();
  const counts = rows.reduce((acc, r) => {
    const [y, m] = r.registeredAt.split("-");
    if (Number(y) === year) acc[m] = (acc[m] || 0) + 1;
    return acc;
  }, {});
...
/** Candidates grouped by the position they signed up for, busiest first. */
const candidatesByPosition = () => {
  const counts = CANDIDATES.reduce((acc, c) => {
    acc[c.positionId] = (acc[c.positionId] || 0) + 1;
    return acc;
  }, {});

  return Object.entries(counts)
    .map(([positionId, count]) => ({ id: positionId, count }))
    .sort((a, b) => b.count - a.count);
};

/** Establishments grouped by type — restaurants, hotels, bakeries, cafés. */
const employersByType = () =>
  Object.entries(employerStats().byType)
    .map(([id, count]) => ({ id, count }))
    .sort((a, b) => b.count - a.count);

/**
 * Everything the statistics screen shows. `days` is the range selector: 7, 30
 * or 90 days back from the fixture's today.
 */
export const registrationStats = (days = 30) => ({
  days,
  candidatesPerDay: perDay(CANDIDATES, days),
  employersPerDay: perDay(EMPLOYERS, days),
  employersPerMonth: perMonth(EMPLOYERS),
  candidatesByPosition: candidatesByPosition(),
  employersByType: employersByType(),
});
...
/**
 * Every administrative action that gets written to the log, with the wording it
 * appears under.
 *
 * A catalogue rather than three strings at each call site: `detail` is a
 * system-generated label, so it has to carry all three languages, and the same
 * action performed from two different screens must read identically in the log.
 *
 * Labels are the bare action. When the action carries a reason, `logAction`
 * appends it in parentheses in the same language — "Photo refusée (contenu
 * inapproprié)" — which is how the seeded entries below already read.
 */
export const ACTION_DETAILS = {
  "chef.verified": { type: "admin", fr: "Profil vérifié", ar: "البروفيل متحقّق منو", en: "Profile verified" },
  "chef.unverified": { type: "admin", fr: "Vérification retirée", ar: "التحقق تحيّد", en: "Verification removed" },
  "chef.edited": { type: "admin", fr: "Profil corrigé", ar: "البروفيل تصلّح", en: "Profile corrected" },
  "chef.skillAdded": { type: "admin", fr: "Compétence ajoutée", ar: "مهارة تزادت", en: "Skill added" },
  "chef.skillRemoved": { type: "admin", fr: "Compétence retirée", ar: "مهارة تحيّدات", en: "Skill removed" },
  "chef.deactivated": { type: "admin", fr: "Profil désactivé", ar: "البروفيل تعطّل", en: "Profile deactivated" },
  "chef.deleted": { type: "admin", fr: "Profil supprimé", ar: "البروفيل تمسح", en: "Profile deleted" },
  "chef.restored": { type: "admin", fr: "Profil rétabli", ar: "البروفيل ترجّع", en: "Profile restored" },
  "chef.blocked": { type: "admin", fr: "Compte candidat bloqué", ar: "حساب المترشح مسدود", en: "Candidate account blocked" },
  "chef.unblocked": { type: "admin", fr: "Compte candidat débloqué", ar: "حساب المترشح تفتح", en: "Candidate account unblocked" },
  "request.approved": { type: "admin", fr: "Établissement approuvé", ar: "المؤسسة تصادق عليها", en: "Establishment approved" },
  "request.rejected": { type: "admin", fr: "Établissement refusé", ar: "المؤسسة مرفوضة", en: "Establishment rejected" },
  "restaurant.blocked": { type: "admin", fr: "Compte bloqué", ar: "الحساب مسدود", en: "Account blocked" },
  "restaurant.unblocked": { type: "admin", fr: "Compte débloqué", ar: "الحساب تفتح", en: "Account unblocked" },
  "job.approved": { type: "admin", fr: "Offre approuvée", ar: "العرض تصادق عليه", en: "Offer approved" },
  "job.rejected": { type: "admin", fr: "Offre rejetée", ar: "العرض مرفوض", en: "Offer rejected" },
  "job.deleted": { type: "admin", fr: "Offre supprimée", ar: "العرض تمسح", en: "Offer deleted" },
  "job.edited": { type: "admin", fr: "Offre modifiée", ar: "العرض تبدّل", en: "Offer edited" },
  "job.extended": { type: "admin", fr: "Offre prolongée exceptionnellement", ar: "العرض تمدّد استثنائيا", en: "Offer extended exceptionally" },
  "job.republished": { type: "admin", fr: "Offre republiée", ar: "العرض تعاود نشر", en: "Offer republished" },
  "job.deactivated": { type: "admin", fr: "Offre désactivée", ar: "العرض تعطّل", en: "Offer deactivated" },
  "photo.approved": { type: "admin", fr: "Photo approuvée", ar: "التصويرة تصادق عليها", en: "Photo approved" },
  "photo.removed": { type: "admin", fr: "Photo refusée", ar: "التصويرة مرفوضة", en: "Photo rejected" },
  "feedback.replied": { type: "admin", fr: "Réponse envoyée au message", ar: "الجواب تصيفط للرسالة", en: "Reply sent to message" },
  "homepage.bannerSet": { type: "admin", fr: "Fond de la page d'accueil modifié", ar: "خلفية الصفحة الرئيسية تبدلات", en: "Homepage background changed" },
  "homepage.bannerCleared": { type: "admin", fr: "Fond de la page d'accueil retiré", ar: "خلفية الصفحة الرئيسية تحيّدات", en: "Homepage background removed" },
  "contact.viewed": { type: "contact-access", fr: "Coordonnées consultées", ar: "الاطلاع على معلومات التواصل", en: "Contact details viewed" },
  "admin.created": { type: "admin", fr: "Administrateur créé", ar: "أدمين تزاد", en: "Administrator created" },
  "admin.permissionsChanged": { type: "admin", fr: "Droits d'administrateur modifiés", ar: "صلاحيات الأدمين تبدلات", en: "Administrator permissions changed" },
  "admin.disabled": { type: "admin", fr: "Administrateur désactivé", ar: "الأدمين تعطّل", en: "Administrator disabled" },
  "admin.enabled": { type: "admin", fr: "Administrateur réactivé", ar: "الأدمين ترجّع", en: "Administrator re-enabled" },
  "candidates.exported": { type: "admin", fr: "Base CV exportée", ar: "قاعدة السي في تصدرات", en: "CV database exported" },
  "candidates.exportedWithContact": { type: "contact-access", fr: "Base CV exportée avec coordonnées", ar: "قاعدة السي في تصدرات مع معلومات التواصل", en: "CV database exported with contact details" },
};
...
/**
 * The rights a sub-admin can hold.
 *
 * `view-contact` is separate from `manage-candidates` on purpose: the client's
 * own example in section 18 is an administrator who manages candidates but
 * cannot see their contact details, and that was not expressible while the two
 * were one permission.
 */
export const PERMISSIONS = [
  ...
  { id: "view-contact", fr: "Voir les coordonnées des candidats", ar: "شوف معلومات التواصل ديال المترشحين", en: "View candidate contact details" },
  { id: "export-cv", fr: "Exporter la base CV", ar: "تصدير قاعدة السي في", en: "Export CV database" },
  { id: "view-statistics", fr: "Consulter les statistiques", ar: "شوف الإحصائيات", en: "View statistics" },
  { id: "manage-feedback", fr: "Répondre aux retours", ar: "جاوب على الملاحظات", en: "Answer feedback" },
  { id: "manage-homepage", fr: "Modifier la page d'accueil", ar: "بدّل الصفحة الرئيسية", en: "Edit the homepage" },
  { id: "view-activity", fr: "Consulter le journal d'activité", ar: "شوف سجل النشاط", en: "View the activity log" },
...
export const ADMINS = [
  ...
  // The client's two other examples from section 18, so the permission model can
  // be demonstrated rather than described: one who manages candidates without
  // seeing their contact details, and one who only reads statistics.
  {
    id: "adm-4",
    name: "Sous-admin — Candidats",
    email: "candidats@nkhedmou.ma",
    role: "sub",
    permissions: ["manage-candidates", "approve-photos"],
    lastActive: "2026-08-15 11:40",
  },
  {
    id: "adm-5",
    name: "Sous-admin — Statistiques",
    email: "stats@nkhedmou.ma",
    role: "sub",
    permissions: ["view-statistics"],
    lastActive: "2026-08-13 09:05",
  },
];
...
export const employerActivity = () =>
  EMPLOYERS.map((e) => {
    ...
    return {
      ...
      // "Reported/declared hires" (ClientDoc 15): applications the employer
      // marked as hired. The outcome is already in the applications fixture.
      declaredHires: APPLICATIONS.filter((a) => a.employerId === e.id && a.status === "hired").length,
